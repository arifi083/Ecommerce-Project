...
<tbody>
	...
	<tr>
      <td><img src="{{ asset($item->slider_img) }}" style="width: 60px; height: 40px;"> </td>
	   <td>
           @if($item->title == NULL)
	...
	<a href="{{ route('slider.inactive',$item->id) }}" class="btn btn-info btn-sm" title="Inactive Now"><i class="fa fa-arrow-down"></i> </a>
		   @else 
	<a href="{{ route('slider.active',$item->id) }}" class="btn btn-info btn-sm" title="Active Now"><i class="fa fa-arrow-up"></i> </a>
		   @endif
